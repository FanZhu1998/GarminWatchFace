...
<p align="center">
  <img src="docs/preview-v3.png" alt="FanFace on the epix Pro (Gen 2) 47 mm: date, time, training status, and a stats panel showing heart rate, Body Battery, and calories" width="360">
  <br>
...
today, large and legible. Calories show as the exact integer (`850`, `2345`), which fits the cell in the
tabular value font.

## Highlights

- **An industrial typographic system.** The whole face is set in DIN 1451 (via the OFL D-DIN revival),
  the German engineering/signage standard, shipped as hand-generated bitmap fonts sized for a 33 mm
  screen: an ~84 px time, ~31 px stat values, and labels no smaller than a readable ~17 px cap height.
  Digits are tabular, so the seconds and changing numbers never jitter. Colors and hairline rules stay
  ported from the site's dark theme: a single steel-blue accent, with green/red used *only* as verdicts
  (a strong Body Battery goes green, a low one red; Training Status shows Productive in green,
  Overreaching in red).
...
```
          SUN · SEP 13                date            eyebrow, tight above the time
           10:08  37                  time            DIN 1451 (D-DIN); seconds receded
   ...
   ──────────┬────────┬──────────     panel top rule (also the coach underline), visible steel
     128     │   82   │   2345        values          accent (Body Battery tints bull/bear)
...
The whole face is set in **DIN 1451**, the German industrial / signage typeface, via **D-DIN** — a
faithful DIN 1451 revival under the SIL Open Font License 1.1 (github.com/amcchord/datto-d-din). The
`.fnt`/`.png` pairs in `resources/fonts/` are generated from the D-DIN TTFs by `tools/FontGen.java`
(BMFont text format, coverage in RGB + alpha, tracking baked into `xadvance`). The OFL text ships in
`resources/fonts/OFL.txt`, and the generated files use the face name `FanFaceDIN` so the derivative does
not carry D-DIN's reserved font name.

Font ids in `fonts.xml` are **role-based** (`TimeFont`, `ValueFont`, `SecFont`, `HeadFont`, `LabelFont`),
not typeface-based, so the typeface can be swapped in future by regenerating these files without touching
source. To regenerate (needs `D-DIN.ttf` and `D-DIN-Bold.ttf`):

```
javac tools/FontGen.java -d tools/out
java -cp tools/out FontGen bmfont D-DIN.ttf      118 "0123456789:"     resources/fonts/din_time  0 s1.2
java -cp tools/out FontGen bmfont D-DIN-Bold.ttf  44 "0123456789,.%-K" resources/fonts/din_value 0 tab
java -cp tools/out FontGen bmfont D-DIN.ttf       30 "0123456789"      resources/fonts/din_sec   0 tab
java -cp tools/out FontGen bmfont D-DIN.ttf       26 "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789:.%-·— " resources/fonts/din_head  1
java -cp tools/out FontGen bmfont D-DIN.ttf       24 "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789:.%-·— " resources/fonts/din_label 1 tab
```

The trailing `tab` gives digits a uniform (centered) advance — tabular figures — so the per-second
seconds and the changing value/footer numbers never jitter with DIN's proportional design. Sizes are
tuned for the 1.3-inch screen: time figures ~84 px (Regular, lightly faux-bolded), rail values ~31 px, date and coach line ~18 px cap
height, labels and footer ~17 px. Calories show as the exact integer (`850`, `2345`); a day's burn stays
within four digits, which fits the rail cell. Every character the face draws must be in the `filter` in `fonts.xml`, or
it renders blank.

## License

MIT for the code (see `LICENSE`). The DIN 1451 typeface is D-DIN under the SIL Open Font License 1.1
(see `resources/fonts/OFL.txt`); the generated bitmap fonts are a derivative distributed under the same
license.
